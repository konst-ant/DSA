...
class BTreeInnerNode<TKey extends Comparable<TKey>> extends BTreeNode<TKey> {
	...
	/* UNCLEAR: if left and right branches are inserted as children corresponding to key with index 'index'
				then where the earlier existing right branch of key in this index position goes?
 		CLEARED: the left child of key with index 'index' is being abandoned, i.e. replaced, because the typical
 				scenario where this method is used is when the old left child branch is split into two new branches
 				(leftChild and rightChild) and they become the actual branches of the new key in index 'index', so
 				the old left child is rewritten with new rightChild
	 */
	...
	protected BTreeNode<TKey> split() {
		...
		for (int i = midIndex + 1; i <= this.getKeyCount(); ++i) {
			...
			/* the next line seems overkill, since the parent link is set from within setChild() call above */
	...
	@Override
	/* UNCLEAR: if the key coincides with some existing key of this node, then search returns the
		index of coincident key, and the we insert into that position shifting tail of keys to the right.
		Does that mean that then we get two keys with equal values in the array of keys?
	 */
